...
@Component
public class ShuUserUtil {
    private static String termPort;
    @Value("${term_port}")
    public static void setTermPort(String termPort) {
        ShuUserUtil.termPort = termPort;
    }

    private static ShuUserUtil shuUserUtil = new ShuUserUtil();
    private ShuUserUtil(){

    }
    public static ShuUserUtil getInstance(){
        return shuUserUtil;
    }
    public Boolean login(User user) {
        OkHttpClient client = user.getClient();
        boolean login = false;
        String relayState="";
        String samlRequest="";
        String samlResponce="";
    ...
    public Boolean logout(User user){
        Request request = new Request.Builder()
                .url("http://xk.autoisp.shu.edu.cn:"+8080+"/Login/Logout")
                .build();
        try{
